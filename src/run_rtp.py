...
def main(
        model_name, chat_template_model_name, model_precision, template_index, probe_type, device, subsets
):
    ...
    if "input" in subsets:
        command = (
            ...
            f"--probing_labels toxicity,profanity,threat,sexually_explicit,identity_attack,insult,length "
        )
    ...
    if "forward" in subsets:
        command = (
            ...
            f"--model_name {model_name} "
            f"--template_index {template_index} "
            f"--control_task {            f"--control_task NONE "
} "
            f"--model_precision {model_precision} "
            f"--probe_type {probe_type} "
    ...
    if "backward" in subsets:
        command = (
            ...
            f"--encoding_folder {encoding_folder} "
            f"--result_folder {result_folder} "
            f"--probing_labels max_toxicity,max_profanity,max_threat,max_sexually_explicit,max_identity_attack,max_insult,length "
        )
