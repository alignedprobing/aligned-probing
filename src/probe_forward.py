...
@click.option('--model_name', type=str, default="google/mt5-small")
...
@click.option('--project_prefix', type=str, default="lift")
...
@click.option('--template_index', type=str, default=2)
@click.option('--num_return_sequences', type=int, default=2)
...
def main(
    ...
    project_prefix = project_prefix + "-cross"
    ...
    for parsed_template_index in template_indices:
        ...
        encoding_files = glob.glob(f"{encoding_folder_structure}/**/generation*.*", recursive=True)
        ...
        for encoding_file in encoding_files:
            ...
            if "feather" in encoding_file:
                ...
                input_frame = pandas.read_feather(f"{encoding_folder_structure}/{layer_name}/input.feather")
            else:
                frame = pandas.read_pickle(encoding_file)
                input_frame = pandas.read_pickle(f"{encoding_folder_structure}/{layer_name}/input.feather")
            ...
            relevant_columns = [col for col in input_frame.columns if col not in frame.columns]
            frame = pandas.concat([
                frame, input_frame[relevant_columns]
            ], axis=1)
            ...
            for probing_label in probing_labels:
                ...
                probe_name = f"{task}-{probing_label}"
